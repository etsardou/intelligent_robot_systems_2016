...
#include "dummy_slam/dummy_slam.h"
...
namespace crsm_slam{
  ...
  CrsmSlam::CrsmSlam(int argc, char **argv){
    ...
    _mapPublishingTimer =
      n.createTimer(ros::Rate(1.0), &CrsmSlam::publishOGM, this);
  ...
  void CrsmSlam::updateParameters(void){
    ...
    n.getParam("/dummy_slam/robot_init_x", slamParams.robot_init_x);
    n.getParam("/dummy_slam/robot_init_y", slamParams.robot_init_y);

    if (n.hasParam("/dummy_slam/occupancy_grid_publish_topic"))
      n.getParam("/dummy_slam/occupancy_grid_publish_topic", slamParams.occupancy_grid_publish_topic);
    else {
      ROS_WARN("[CrsmSlam] : Parameter occupancy_grid_publish_topic not found. Using Default");
      slamParams.occupancy_grid_publish_topic = "/dummy_slam/map" ;
    }

    if (n.hasParam("/dummy_slam/robot_trajectory_publish_topic"))
      n.getParam("/dummy_slam/robot_trajectory_publish_topic", slamParams.robot_trajectory_publish_topic);
    else {
      ROS_WARN("[CrsmSlam] : Parameter robot_trajectory_publish_topic not found. Using Default");
      slamParams.robot_trajectory_publish_topic = "/dummy_slam/trajectory" ;
    }

    if (n.hasParam("/dummy_slam/trajectory_publisher_frame_id"))
      n.getParam("/dummy_slam/trajectory_publisher_frame_id", slamParams.trajectory_publisher_frame_id);
    else {
    ...
    if (n.hasParam("/dummy_slam/laser_subscriber_topic"))
      n.getParam("/dummy_slam/laser_subscriber_topic", slamParams.laser_subscriber_topic);
    else {
      ROS_WARN("[CrsmSlam] : Parameter laser_subscriber_topic not found. Using Default");
      slamParams.laser_subscriber_topic = "/dummy_slam/laser_scan" ;
    }

    if (n.hasParam("/dummy_slam/world_frame"))
      n.getParam("/dummy_slam/world_frame", slamParams.world_frame);
    else {
      ROS_WARN("[CrsmSlam] : Parameter world_frame not found. Using Default");
      slamParams.world_frame = "world" ;
    }

    if (n.hasParam("/dummy_slam/base_footprint_frame"))
      n.getParam("/dummy_slam/base_footprint_frame", slamParams.base_footprint_frame);
    else {
    ...
    if (n.hasParam("/dummy_slam/base_frame"))
      n.getParam("/dummy_slam/base_frame", slamParams.base_frame);
    else {
      ROS_WARN("[CrsmSlam] : Parameter base_frame not found. Using Default");
      slamParams.base_frame = "base_link" ;
    }

    if (n.hasParam("/dummy_slam/map_frame"))
      n.getParam("/dummy_slam/map_frame", slamParams.map_frame);
    else {
      ROS_WARN("[CrsmSlam] : Parameter map_frame not found. Using Default");
      slamParams.map_frame = "map" ;
    }

    if (n.hasParam("/dummy_slam/laser_frame"))
      n.getParam("/dummy_slam/laser_frame", slamParams.laser_frame);
    else {
      ROS_WARN("[CrsmSlam] : Parameter laser_frame not found. Using Default");
      slamParams.laser_frame = "laser_link" ;
    }

    if (n.hasParam("/dummy_slam/hill_climbing_disparity"))
      n.getParam("/dummy_slam/hill_climbing_disparity", slamParams.disparity);
    else {
      ROS_WARN("[CrsmSlam] : Parameter hill_climbing_disparity not found. Using Default");
      slamParams.disparity = 40 ;
    }

    if (n.hasParam("/dummy_slam/slam_container_size"))
      n.getParam("/dummy_slam/slam_container_size", slamParams.map_size);
    else {
      ROS_WARN("[CrsmSlam] : Parameter slam_container_size not found. Using Default");
      slamParams.map_size = 500 ;
    }

    if (n.hasParam("/dummy_slam/slam_occupancy_grid_dimentionality"))
      n.getParam("/dummy_slam/slam_occupancy_grid_dimentionality", slamParams.ocgd);
    else {
      ROS_WARN("[CrsmSlam] : Parameter slam_occupancy_grid_dimentionality not found. Using Default");
      slamParams.ocgd = 0.02 ;
    }

    if (n.hasParam("/dummy_slam/map_update_density"))
      n.getParam("/dummy_slam/map_update_density", slamParams.density);
    else {
      ROS_WARN("[CrsmSlam] : Parameter map_update_density not found. Using Default");
      slamParams.density = 30.0 ;
    }

    if (n.hasParam("/dummy_slam/map_update_obstacle_density"))
      n.getParam("/dummy_slam/map_update_obstacle_density", slamParams.obstacle_density);
    else {
      ROS_WARN("[CrsmSlam] : Parameter map_update_obstacle_density not found. Using Default");
      slamParams.obstacle_density = 3.0 ;
    }

    if (n.hasParam("/dummy_slam/scan_density_lower_boundary"))
      n.getParam("/dummy_slam/scan_density_lower_boundary", slamParams.scan_selection_meters);
    else {
      ROS_WARN("[CrsmSlam] : Parameter scan_density_lower_boundary not found. Using Default");
      slamParams.scan_selection_meters = 0.3 ;
    }

    if (n.hasParam("/dummy_slam/max_hill_climbing_iterations"))
      n.getParam("/dummy_slam/max_hill_climbing_iterations", slamParams.max_hill_climbing_iterations);
    else {
    ...
    if (n.hasParam("/dummy_slam/occupancy_grid_map_freq"))
      n.getParam("/dummy_slam/occupancy_grid_map_freq", slamParams.occupancy_grid_map_freq);
    else {
      ROS_WARN("[CrsmSlam] : Parameter occupancy_grid_map_freq not found. Using Default");
      slamParams.occupancy_grid_map_freq = 1.0 ;
    }

    if (n.hasParam("/dummy_slam/robot_pose_tf_freq"))
      n.getParam("/dummy_slam/robot_pose_tf_freq", slamParams.robot_pose_tf_freq);
    else {
      ROS_WARN("[CrsmSlam] : Parameter robot_pose_tf_freq not found. Using Default");
      slamParams.robot_pose_tf_freq = 5.0 ;
    }

    if (n.hasParam("/dummy_slam/trajectory_freq"))
      n.getParam("/dummy_slam/trajectory_freq", slamParams.trajectory_freq);
    else {
    ...
    if (n.hasParam("/dummy_slam/desired_number_of_picked_rays"))
      n.getParam("/dummy_slam/desired_number_of_picked_rays", slamParams.desired_number_of_picked_rays);
    else {
    ...
    if (n.hasParam("/dummy_slam/robot_width"))
      n.getParam("/dummy_slam/robot_width", slamParams.robot_width);
    else {
      ROS_WARN("[CrsmSlam] : Parameter robot_width not found. Using Default");
      slamParams.robot_width = 0.5 ;
    }

    if (n.hasParam("/dummy_slam/robot_length"))
      n.getParam("/dummy_slam/robot_length", slamParams.robot_length);
    else {
  ...
  void CrsmSlam::updateMapProbabilities(void){
    ...
    // HACK!!!! Read tf and directly update map
    tf::StampedTransform trans;
    _listener.lookupTransform(slamParams.map_frame, "robot0", ros::Time(0), trans);
    tf::Matrix3x3 m(trans.getRotation());
    double roll, pitch, yaw;
    m.getRPY(roll, pitch, yaw);

    double xx = (trans.getOrigin().x() - slamParams.robot_init_x) / slamParams.ocgd;
    double yy = (trans.getOrigin().y() - slamParams.robot_init_y) / slamParams.ocgd;
    double th = yaw;
    //ROS_ERROR_STREAM(xx << " " << yy << " " << yaw << "\n");
    //ROS_ERROR_STREAM(robotPose.x << " - " << robotPose.y << " " << robotPose.theta << "\n");
    robotPose.x = xx;
    robotPose.y = yy;
    robotPose.theta = yaw;
  ...
  /**
    ...
    @return void
   **/
  void CrsmSlam::publishOGM(const ros::TimerEvent& event){
    ...
    //ROS_ERROR_STREAM(width << " " << height << " " << grid.info.origin.position.x << " " << grid.info.origin.position.y);
